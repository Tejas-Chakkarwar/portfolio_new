...
export default function Home() {
    ...
    <div className="projects-container" style={{ paddingTop: '0px' }}>
      <div className="project-grid">
        ...
        <div className="project-card" style={{ gridColumn: 'span 2', maxWidth: '100%' }}>
          <div className="project-content">
            <div className="project-header">
              <h3 className="project-title">🧩 Code & Infra Highlights</h3>
            </div>
            <div className="project-description" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                <strong>Lambda Config (Terraform)</strong>
                <pre className="code-block"><code>{`resource "aws_lambda_function" "start_campaign" {
  ...
  environment {
    variables = {
      ...
      EVENTBRIDGE_ROLE_ARN        = var.scheduler_invoke_role_arn
              ...
              <div>
                <strong>Lambda Handler (SQS trigger)</strong>
                <pre className="code-block"><code>{`def lambda_handler(event, _context):
    # Records: {"campaign_id":123, "recipient_id":456, "email":"user@example.com"}
    for rec in event.get("Records", []):
        ...
        # process send...`}</code></pre>
              </div>

              <div>
                <strong>SQS Event Source Mapping (Terraform)</strong>
                <pre className="code-block"><code>{`resource "aws_lambda_event_source_mapping" "send_worker_sqs" {
  ...
  scaling_config { maximum_concurrency = local.ses_max_concurrency } # 2 × 7 = ~14 emails/sec
}`}</code></pre>
              </div>

              <div>
                <strong>Fan-out to SQS (start_campaign)</strong>
                <pre className="code-block"><code>{`for batch in _chunks(contacts, 10):
    entries = []
    for c in batch:
        message_body = {
    ...
    sqs.send_message_batch(QueueUrl=SQS_URL, Entries=entries)`}</code></pre>
              </div>

              <div style={{ display: 'grid', gap: '12px', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
                <div>
                  <strong>DynamoDB Tables (GSIs)</strong>
                  <ul className="project-highlights">
                    <li>Users: GSI on email, api_key</li>
                    <li>Campaigns: GSI owner_index</li>
                    <li>Events: GSI campaign_index</li>
                    <li>Link mappings: GSI campaign_recipient_index + TTL (expires_at)</li>
                  </ul>
                </div>
                <div>
                  <strong>SQS Queues</strong>
                  <pre className="code-block"><code>{`resource "aws_sqs_queue" "dlq" {
  name = "\${var.name}-dlq"
  ...
  redrive_policy = jsonencode({
    ...
    maxReceiveCount     = 3
                ...
                <div>
                  ...
                  <pre className="code-block"><code>{`def exponential_backoff_retry(func, max_retries=3, base_delay=1.0, max_delay=60.0):
    last_exception = None
